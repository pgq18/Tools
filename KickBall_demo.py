...
colors = [[103, 84, 160], [40, 165, 152], [9, 153, 88]] # green, red, blue

min_colors = [[
    int(color[0] - 50 * threshold * 2),
    int(color[1] - 50 * threshold),
    int(color[2] - 50 * threshold)
    ] for color in colors]
max_colors = [[
    int(color[0] + 50 * threshold * 2),
    int(color[1] + 50 * threshold),
...
def run(img):
    ...
    if target_color == []:
        ...
        frame_mask = cv2.inRange(frame_lab, tuple(min_colors[0]), tuple(max_colors[0]))
        frame_mask = cv2.inRange(frame_lab, tuple(min_colors[1]), tuple(max_colors[1])) | frame_mask
        frame_mask = cv2.inRange(frame_lab, tuple(min_colors[2]), tuple(max_colors[2])) | frame_mask
        #腐蚀(corrosion)
        eroded = cv2.erode(frame_mask, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)))  
        #膨胀(dilation)
        dilated = cv2.dilate(eroded, cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)))
        # 找出轮廓(find out contour)
        ...
        if areaMaxContour is not None:  # 如果找到最大面积的轮廓(if the contour with the maximal area is found)
            try:
                ...
                target_color = [frame_lab[int(CenterY)][int(CenterX)], [0,0,0]]
...
@app.route('/tonypi/api/kick_ball', methods=['POST'])
def command():
...
if __name__ == '__main__':
    ...
    init()
    loop_th = threading.Thread(target=loop)
    loop_th.daemon = True
    loop_th.start()
